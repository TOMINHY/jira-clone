...
const CreateProjectPage = () => {
  ...
  const { projectCategory } = useSelector((state) => state.auth);
  const [projectCategories, setProjectCategoris] = useState([]);
  useEffect(() => {
    if (projectCategory) {
      setProjectCategoris(projectCategory);
    }
  }, [projectCategory]);
  const handleCreatProject = async (values) => {
    if (!isValid) return;
  ...
  return (
    <div className=" min-h-screen flex justify-start">
      ...
      <div className="p-10 flex-1">
        <h1 className="text-3xl font-medium">CreateProject</h1>
